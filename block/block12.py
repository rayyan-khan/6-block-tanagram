...
def solve(board, currentRow, nextOpenIndex, choices):
    if areaAddsUp(BOARDH, BOARDW, BLOCKS):
        return output(solve1(board, currentRow, nextOpenIndex, choices))
    else:
        return 'Blocks do not fill area.'

# method to find puzzle solution:
def solve1(board, currentRow, nextOpenIndex, choices):
    if not choices:
        if filled(board):
            print(board)
            return board
    ...
    for choice in choices:
        ...
        for rotation in range(2):
            if rotation == 0:
                ...
                if newBoard:
                    result = solve1(newBoard, currentRow, nextOpenIndex, newChoices)
                    if result:
                        return result
