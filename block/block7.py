...
def solve1(board, currentRow, choices, blocksIn):
    ...
    for choice in choices:
        ...
        for rotations in ((0, 1), (1, 0)):
            y, x = rotations
            height, width = int(BLOCKS[choice][y]), int(BLOCKS[choice][x])
            newBoard = addBlock(height, width, choice, (nextOpenIndex, currentRow), board, newBlocksIn)
            if newBoard == False:
                continue # maybe try rotating here, but have to account for output then
